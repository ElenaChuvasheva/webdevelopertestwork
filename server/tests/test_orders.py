...
def test_cancel_order(place_normal_order_message):
    ...
    with client.websocket_connect("/ws/") as websocket:
        ...
        websocket.send_text(f'{{"messageType": 4, "message": {{"orderId": "{uuid}"}}}}')
        data = websocket.receive_json()
        assert data['messageType'] == 3
...
def test_normal_order_to_base(place_normal_order_message):
    ...
    with client.websocket_connect("/ws/") as websocket:
        ...
        data = websocket.receive_json()
        uuid = data['message']['orderId']
        websocket.send_text(f'{{"messageType": 6, "message": {{"orderId": "{uuid}"}}}}')
        data = websocket.receive_json()
...
def test_noexist_order_to_base():
    client = TestClient(api)
    with client.websocket_connect("/ws/") as websocket:
        websocket.send_text(r'{"messageType": 6, "message": {"orderId": "4a01e7fc-4cb5-4718-b563-9d049c6b0272"}}')
        data = websocket.receive_json()
        assert data['messageType'] == 2
...
def test_get_orders(place_normal_order_message, get_all_orders_message):
    ...
    with client.websocket_connect("/ws/") as websocket:
        ...
        fields = ('creationTime', 'changeTime', 'status', 'side', 'price', 'amount', 'instrument', 'uuid')
